...
// pug 파일 동기적으로 로드.
const registerpug = fs.readFileSync(path.join(__dirname, "../views/register.pug"), 'utf8');
const registerCompletepug = fs.readFileSync(path.join(__dirname, "../views/register-complete.pug"), 'utf8');
    ...
    form.parse(req, function(err, body, files) {
        ...
        if (req.device.type == "desktop") {
            ...
            if (req.session.reg_certified) {
                ...
                ciResult += ci.final('hex');
        
                // DB에 이미 아이디가 겹치는게 존재하는지 체크함.
                connection.query('SELECT * FROM USER WHERE USER_ID LIKE ' + '"' + body.userid + '"', (err, results) => {

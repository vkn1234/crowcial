...
router.get('/',function(req,res){
    fs.readFile(path.join(__dirname, "../views/CompleteList.pug"), 'utf8', function (err, data) {
        if (err) {
            console.log("CompleteList.pug 파일 오류");
        } else {
            ...
            client.query(query,params,(err,rows)=>{
                res.render('CompleteList.pug',{ rows: rows });
            })
        }
    });
});
    ...
    client.query(query,params,(err,rows)=>{
            ...
            else    
        {
            if (rows.length > 0){
                ...
                var filePath = __dirname + "/../project/spendmoney/" +filename; // 다운로드할 파일의 경로​    ../views/user-complete.pug  

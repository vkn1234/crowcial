...
const router = express.Router();// 이 라우터 모듈은 접속한 사용자가 desktop 유저인지 phone 유저인지를 비교하는 모듈을 사용함.
...
// pug 파일 동기적으로 로드.
const pugFile = fs.readFileSync(path.join(__dirname, "../views/login.pug"), 'utf8');// 사용자에게 login.pug 파일을 보여준다.
router.get('/', (req, res) => {
...
router.post('/', (req, res) => {
    ...
    connection.query('SELECT * FROM USER WHERE USER_ID LIKE ' + '"' + req.body.userid + '"', (err, results) => {
        if (results.length > 0) {
                ...
                if (results[0].USER_STOP != 1) {
                    if (req.device.type == 'desktop') {
                        ...
                        connection.query(query, params, (err, rows) => {
                            ...
                            res.json({
                                sessionId: req.sessionID,userid: req.body.userid,username: results[0].USER_NAME,usernum: results[0].USER_NUM,imageName: results[0].USER_IMAGE, LOGIN_COMPLETE: true, WRONG_PASSWORD: false, UNKNOWN_USERID: false
                                });
                            });
                        }
                    } else {

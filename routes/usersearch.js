...
// pug 파일 동기적으로 로드.
const usersearchpug = fs.readFileSync(path.join(__dirname, "../views/usersearch.pug"), 'utf8');
const newpasswordpug = fs.readFileSync(path.join(__dirname, "../views/usersearch-newpassword.pug"), 'utf8');
const newpasswordCompletepug = fs.readFileSync(path.join(__dirname, "../views/usersearch-newpassword-complete.pug"), 'utf8');
...
router.get('/', (req, res) => {
    ...
    res.status(200).send(pug.render(usersearchpug));
});
...
router.get('/pw/new', (req, res) => {
    ...
    if (req.device.type == 'desktop') {
        if (req.session.search_userid) {
            res.status(200).send(pug.render(newpasswordpug, {
...
router.post('/pw/new', (req, res) => {
    ...
    if (req.device.type == 'desktop') {
        ...
        connection.query(query, params, (err, rows, fields) => {
            if (!err) {
                res.status(200).send(pug.render(newpasswordCompletepug));
            } else {
